...
class GradCAM(nn.Module):
    ...
    def __init__(self, model, layer='layer4'):
        ...
        self.model = model
        self.device = next(self.model.parameters()).device # Getting device on which model is present
        try:
            self.layer = getattr(self.model, layer)
        except AttributeError:
            print(f"Couldn't find the the {layer} in {model}, using the Last Layer {model[-1]}")
            self.layer = model[-1]
